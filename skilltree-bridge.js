/**
 * skilltree-bridge.js — Fatbody Framework (Modern RPG mode, v3.0)
 *
 ...
 *
 * Imports: state-manager.js, skilltree-protocol.js, memo-processor.js, env.js
...
export function openSkillTreeTab() {
    ...
    if (getCampaignMode(chatId) !== 'modern') {
        toastr['info']('The Skill Tree is a Modern-mode feature. Build a foundation first (Foundation Builder).', 'Skill Tree');
        return;
    }
    const st = getSettings().chatStates?.[chatId];
    if (!st?.progression) {
    ...
    const win = window.open(url, 'fatbody-skilltree');
    if (!win) {
