...
Health: ((BAR:45/100))
((OBJ)) Slay the dragon
((OBJ:done)) Rescue the villagers
((REWARD)) 250 Gold
((DIFFICULTY)) Hard
((PROGRESS)) Mushrooms collected: 3/5`;
...
If usage state changed, re-output the ENTIRE [SKILLS] block.`,
    origin: `Character origin profile (Origins campaigns). This block is written by the engine at character creation and is CANON — NEVER add, remove, rename, or rewrite its lines. The "Appearance" line is the character's established physical description: describe them consistently with it and never restate or revise it here. The "World-Threat Tie-In" line appears only once a World Arc has been compiled; never add one yourself. Your ONLY permitted change is the "Current Goal" line, and only when the narrative has clearly and explicitly changed what the character wants right now. If you update it, re-output the ENTIRE [ORIGIN] block with only that line changed. Otherwise omit this block from your output entirely.`,
...
SCALING TO QUEST DIFFICULTY (only when the current scene is tied to an active tracked quest that has an assigned difficulty):
- Very Easy / Easy: pick enemies below or at the low end of {{user}}'s current level — comfortably winnable, more texture than threat.
- Medium: pick enemies roughly at {{user}}'s current level — a fair, competitive fight.
- Hard: enemies may run meaningfully above {{user}}'s level — dangerous, but winnable with good tactics and resource use.
- Very Hard: enemies can be brutally strong, pushing toward the top of a tier or into Legendary — victory should demand near-perfect play or a creative non-combat solution.

GENERAL / NON-QUEST ENCOUNTERS: for freeform or incidental combat with no tracked quest difficulty attached, do NOT force level-matching — pick whatever tier fits the scene and story logic. The scaling guidance above applies only to quest-tied encounters.
</combat>
...
Save ranges by tier:
  ...
  Legendary — +8 to +12; overwhelming across the board, no weak saves
...
- CONFLICTING SOURCES: the [ORIGIN] block and the origin canon lorebook entry are engine-written and authoritative. A lore entry under "ACTIVE LORE" or "NEWLY ACTIVATED LORE" is model-written and may have drifted. Where the two disagree about a person, place, or faction — species, age, status, allegiance, whether they are alive — the origin canon wins; narrate from it. Do not silently pick the more convenient version, and do not average them. If the drifted detail has already been narrated in this chat, treat the discrepancy as an in-fiction question worth raising (a mistaken record, a rumor, a deliberate deception) rather than retconning it away.
